...
class TestDateField(unittest.TestCase):
    ...
    def setUp(self):
        self.fields = [api.DateField(False, False), api.DateField(False, True),
                       api.DateField(True, False),  api.DateField(True, True)]
    ...
    def test_validate_on_correct_content(self):
        ...
        # unreqired fields
        for f in (api.DateField(False, False), api.DateField(False, True)):
            self.assertIsNone(f.validate(None), msg="on None")
        # could be empty fields
        for f in (api.DateField(False, True), api.DateField(True, True)):
        ...
        for f in self.fields:
            for val in self.invalid_values:
                with self.assertRaises(api.ValidationError,
                    ...
                    f.validate(val)
        # required fields
        for f in (api.DateField(True, False), api.DateField(True, True)):
            with self.assertRaises(api.ValidationError, msg=" on None"):
                f.validate(None)
        # empty fields
        for f in (api.DateField(False, False), api.DateField(True, False)):
            for val in self.null_values:
